...
var geometry = 
    ...
    ee.Geometry.Polygon(
        [[[-51.31461544421311, -29.70289714383088],
          [-51.31461544421311, -30.231120865137083],
          [-50.379404262572486, -30.231120865137083],
          [-50.379404262572486, -29.70289714383088]]], null, false);
...
// Função para processar cada banda, selecionando apenas a classe de interesse (área urbana) e definindo metadados.
var listImages = list.map(function(number) {
  var matchKey = ee.String(ee.List(mapbiomas.bandNames()).get(number))
  var image = ee.Image(mapbiomas).select(matchKey)
...
var collection_vis = imageCollection.map(function(image) {
  ...
  var label = text.draw(image.get('data'), pt, scale, textVis)
  var Titulo = text.draw('Evolucao da Area Urbana na Bacia do rio Gravatai', titulo, scale, textVis)
  var Autor = text.draw('Autor: Christhian Cunha', autor_titulo, scale, textVis)

  // Visualiza a imagem de fundo e a classe de interesse.
  var urban = image.eq(24).selfMask().visualize({palette: ['Crimson']})
  var water = image.eq(33).selfMask().visualize({palette: ['#0000FF']})
  var background = img.visualize({'bands': ['B4', 'B3', 'B2'], 'min': 155, 'max': 2540, 'gamma': 1.65})

  // Combina as imagens e textos.
  return background.blend(urban).blend(water).blend(label).blend(Titulo).blend(Autor);
});
